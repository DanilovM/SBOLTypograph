...
var onRun = function(context) {
    ...
    function runTypograph(stringToParse) {
        ...
        function numbers() {
            // Разбить длинные числа по разрядам
            stringToParse = stringToParse.replace(/([^\u002E\u002C])(\d)(?=(\d{3})+([\D]|$))/gm, function(match, p1, p2) {
                _counterAddNoBreakSpace++
                return p1 + p2 + _nbsp
            })

            // Если после четырёхзначного числа, формата 1 111 не идёт знак валюты или млн., трлн и т.д.
            // Убираем в числе пробел, возможно это год
            stringToParse = stringToParse.replace(/([\u00A0\u0020]\d)\u00A0(\d{3})(?!\u00A0(тыс|млн|млрд|трлн|₽|\$|€|£|¥))/gm, function(match, p1, p2) {
                _counterAddNoBreakSpace--
                return p1 + p2
            })
            ...
            stringToParse = stringToParse.replace(/(\d)\u002E(\d+)([^\d\u002E]|$)/gm, function(match, p1, p2, p3) {
